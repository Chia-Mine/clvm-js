...
export type TOpStack = Function[];
export type TValStack = Array<CLVMObject|Bytes|CLVMObject[]|Tuple2<CLVMObject, Bytes>>;
...
export function as_javascript(sexp: SExp){
  function _roll(op_stack: Function[], val_stack: TValStack){
    const v1 = val_stack.pop() as CLVMObject|Bytes|Tuple2<CLVMObject, Bytes>;
    const v2 = val_stack.pop() as CLVMObject|Bytes|Tuple2<CLVMObject, Bytes>;
    val_stack.push(v1);
    val_stack.push(v2);
  }
  
  function _make_tuple(op_stack: TOpStack, val_stack: TValStack){
    const left = val_stack.pop() as CLVMObject;
    const right = val_stack.pop() as Bytes;
  ...
  function _extend_list(op_stack: TOpStack, val_stack: TValStack){
    ...
    left.concat(right);
    val_stack.push(left as CLVMObject[]);
  }
